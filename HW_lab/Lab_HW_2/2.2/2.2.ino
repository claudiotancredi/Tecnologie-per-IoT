...
/*Global variables*/
volatile int green_led_state = LOW;
...
void presence_detected() {
  if (millis() - timestamps_last_10_minutes[(tail - 1 + N_SOUND_EVENTS) % N_SOUND_EVENTS] > 50) {
    if (millis() - timestamps_last_10_minutes[(tail - 1 + N_SOUND_EVENTS) % N_SOUND_EVENTS] < 500 && millis() - timestamps_last_10_minutes[(tail - 2 + N_SOUND_EVENTS) % N_SOUND_EVENTS] > 500) {
...
void setup() {
  ...
  digitalWrite(GREEN_LED_PIN, MIN);
